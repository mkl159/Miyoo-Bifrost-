...
- PC **Windows** ou **macOS** (Linux non testé)
- Carte SD formatée en **FAT32** (obligatoire — le firmware Miyoo ne supporte pas exFAT)
  - L'installateur peut formater automatiquement quelle que soit la taille
  - Alternative manuelle : **[Rufus](https://rufus.ie)** → FAT32, taille d'unité 32 Ko
...
**Windows — Installateur EXE (recommandé) :**

1. Double-clique sur `INSTALLER_BIFROST.exe`
2. Accepte la demande d'élévation UAC (administrateur requis pour diskpart)
3. Choisis la langue (FR / EN / ES)
...
**Windows — Installateur PowerShell (alternatif) :**

1. Clic droit sur `INSTALLER_SD.ps1` → **Exécuter avec PowerShell**
   > Si Windows bloque : clic droit → Propriétés → coche **Débloquer** → OK
2. Choisis la langue et sélectionne les 3 dossiers

**macOS :**

1. Ouvre le Terminal dans le dossier `Miyoo-Bifrost`
2. Lance : `bash install_macos.sh`
...
> Les boutons utilisables dans les séquences : `UP DOWN LEFT RIGHT X Y L1 R1 START SELECT`
...
| `INSTALLER_BIFROST.exe` | **Installateur Windows EXE** — interface graphique |
| `INSTALLER_BIFROST.cs` | Code source C# de l'installateur (recompiler via `BUILD_EXE.bat`) |
...
- **Windows** or **macOS** PC (Linux untested)
- SD card formatted as **FAT32** (mandatory — Miyoo firmware does not support exFAT)
  - The installer can format automatically for any card size
  - Manual alternative: **[Rufus](https://rufus.ie)** → FAT32, 32 KB cluster size
- **OnionOS**: [github.com/OnionUI/Onion/releases](https://github.com/OnionUI/Onion/releases)
- **TelmiOS**: [github.com/DantSu/Telmi-story-teller/releases](https://github.com/DantSu/Telmi-story-teller/releases)
...
1. Double-click `INSTALLER_BIFROST.exe`
2. Accept the UAC elevation prompt (administrator required for diskpart)
...
**Windows — PowerShell installer (alternative):**

1. Right-click `INSTALLER_SD.ps1` → **Run with PowerShell**
   > If Windows blocks it: right-click → Properties → check **Unblock** → OK
2. Choose language and select the 3 folders

**macOS:**

1. Open Terminal in the `Miyoo-Bifrost` folder
2. Run: `bash install_macos.sh`
...
> Buttons allowed in sequences: `UP DOWN LEFT RIGHT X Y L1 R1 START SELECT`
...
| `INSTALLER_BIFROST.exe` | **Windows EXE installer** — graphical interface |
| `INSTALLER_BIFROST.cs` | C# source code for the installer (recompile via `BUILD_EXE.bat`) |
